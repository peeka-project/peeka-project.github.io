...
- v0.1.14 起，附加过程会显示 `Progress` 与 `Attach Log` 区域，失败时直接在面板内展示真实错误；错误状态下按 Esc 会重置面板，附加进行中按 Esc 不会中断，空闲时按 Esc 退出
...
Peeka v0.1.9–v0.1.14 对附加可靠性进行了多项增强：
- TUI 附加面板新增进度、日志、耗时和内联错误展示（v0.1.14：`56fa814`、`4a300f9`）
- 修复原生 `_socket.socket` 的 accept 调用，提升 monkey-patched 运行时中的 notify server 稳定性（v0.1.14：`5617058`）
...
| 版本 | 发布日期 | 变更说明 |
|------|----------|----------|
| 0.1.14 | 2026-05-24 | TUI 附加进度/日志/内联错误展示；修复 notify server 原生 socket accept |
...
- [patch-status]({% link commands/patch-status.md %}) - 检查目标运行时补丁状态
