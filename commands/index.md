...
Peeka 提供了一系列强大的诊断命令，每个命令都专注于特定的诊断场景。本文档涵盖了 21 个顶层 CLI 命令。
{: .fs-6 .fw-300 }
## 目录
{: .no_toc .text-delta }
...
| [patch-status]({% link commands/patch-status.md %}) | 运行时补丁诊断 | 检查 gevent/eventlet、stdlib 原语和 RPL 完整性 |
...
| [target]({% link commands/target.md %}) | 目标管理 | 发现、检查、清理和分离 Peeka target agent |
| [client]({% link commands/client.md %}) | 客户端会话管理 | 创建、列出、检查和关闭诊断客户端 |
| [job]({% link commands/job.md %}) | 命令任务管理 | 查看、检查、中断和清理异步命令任务 |
| [probe]({% link commands/probe.md %}) | 探针运行管理 | 查看、检查、停止和清理 watch/trace 等探针 |
| [consumer]({% link commands/consumer.md %}) | 结果消费者管理 | 为 job/probe/target 创建和读取缓冲结果流 |
| [dx]({% link commands/dx.md %}) | 诊断案例包 | 创建、汇总和导出诊断案例 |

`session` 是 `target` 的兼容别名，已弃用；新用法请使用 `peeka-cli target ...`。
...
| `logger` | 0% | 不影响性能 |
| `memory` | 可配置 | 取决于采样频率 |
| `patch-status` | 近似 0% | 仅读取运行时状态，不修改目标进程 |
| `target` / `client` / `job` / `probe` / `consumer` / `dx` | 低 | 控制面查询或元数据管理，实际开销取决于读取/导出的数据量 |
...
- [patch-status - 运行时补丁诊断]({% link commands/patch-status.md %})
- [detach - 断开连接]({% link commands/detach.md %})
- [run - 启动并附加]({% link commands/run.md %})
- [target - 目标管理]({% link commands/target.md %})
- [client - 客户端会话管理]({% link commands/client.md %})
- [job - 命令任务管理]({% link commands/job.md %})
- [probe - 探针运行管理]({% link commands/probe.md %})
- [consumer - 结果消费者管理]({% link commands/consumer.md %})
- [dx - 诊断案例包]({% link commands/dx.md %})
