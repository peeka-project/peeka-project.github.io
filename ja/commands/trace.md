...
![Peeka Trace 呼び出しツリー]({{ site.url }}/assets/images/screenshots/peeka-trace.png)
...
**gevent 互換性（v0.1.16+）**：target プロセスで gevent monkey patch または active hub が検出された場合、`trace` は `sys.settrace` が frame stack 不変条件を壊すことを避けるため `wrapper_only` バックエンドに退化します。このモードでも target 関数の観測結果は報告されますが、再帰的な呼び出しツリーは提供されません。
...
| バージョン | リリース日 | 変更内容 |
|----------|----------|---------|
| 0.1.16 | 2026-06-07 | `--client` を追加。gevent patched/active hub ランタイムでは `wrapper_only` trace バックエンドに退化 |
