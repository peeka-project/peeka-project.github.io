...
**优势**：
- 官方支持，安全可靠
- Python 3.14+ 无需 GDB/LLDB 等外部调试器
- 跨平台兼容

#### Python 3.8.1-3.13

使用调试器降级方案：Linux 使用 GDB + ptrace，macOS 使用 LLDB + dlopen。下面示例展示 Linux 的传统 GDB 路径：
...
**要求**：
- Linux: GDB 7.3+、Python 调试符号、CAP_SYS_PTRACE 权限
- macOS: Xcode Command Line Tools（提供 LLDB）
...
**性能对比**：
- `sys.monitoring`: < 5% 开销
- `sys.settrace`: < 20% 开销（Python 3.8.1-3.11）
