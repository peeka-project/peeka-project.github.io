...
- v0.1.14 以降、アタッチ中は `Progress` と `Attach Log` 領域を表示し、失敗時は実際のエラーをパネル内に表示します。エラー状態の Esc はパネルをリセットし、アタッチ中の Esc は無視され、アイドル時の Esc は終了します
...
Peeka v0.1.9–v0.1.14 では、アタッチの信頼性に関していくつかの強化が行われました：
- TUI アタッチパネルに進捗、ログ、経過時間、インラインエラー表示が追加されました（v0.1.14: `56fa814`、`4a300f9`）
- monkey patch されたランタイムで notify server が安定するよう、ネイティブ `_socket.socket` の accept 処理を修正しました（v0.1.14: `5617058`）
- 接続エラーメッセージがより詳細になりました（v0.1.11: `88da13e`）
- ストリーミングクライアント識別が改善され、接続失敗が減少しました（v0.1.9: `a90d080`）
- Socket 処理と接続検証が強化されました（v0.1.11–v0.1.12: `9ef3222`）
- ブロードキャストフレームが正しくスキップされ、ストリーミングクライアントの安定性が向上しました（v0.1.12: `9c90675`）
...
| バージョン | リリース日 | 変更内容 |
|-----------|-----------|----------|
| 0.1.14 | 2026-05-24 | TUI アタッチの進捗/ログ/インラインエラー表示、notify server のネイティブ socket accept 修正 |
| 0.1.12 | 2026-05-08 | Socket 処理の強化（`9ef3222`）、ブロードキャストフレームのスキップ（`9c90675`） |
| 0.1.11 | 2026-05-07 | アタッチ信頼性の修正、エラー表示の改善（`88da13e`） |
| 0.1.9 | 2026-05-04 | ストリーミングクライアント識別の改善（`a90d080`） |
...
- [patch-status]({% link commands/patch-status.md %}) - 対象ランタイムのパッチ状態を確認
