...
### watch --times パラメータのカウント動作が v0.1.13 で変更

**症状**: v0.1.13 にアップグレード後、`watch --times N` のカウント動作が以前のバージョンと異なります。

**原因**: v0.1.13 は `--times` カウントロジックをエージェント側からクライアント側に移動しました。以前のバージョンはターゲットプロセス内でカウントしていましたが、新バージョンはクライアント側でオブザベーションをカウントします。

**影響**:
- クライアントの切断・再接続後、カウントがリセットされる
- 複数のクライアントがそれぞれ独立して `--times` 制限を設定できる

**解決策**:

これは予想される動作変更です。詳細な動作変更と使用シナリオについては、[watch コマンドドキュメント](../commands/watch.md)を参照してください。

### Attach の信頼性向上（v0.1.9-v0.1.12）

**改善内容**: v0.1.9 から v0.1.12 バージョンでは、attach コマンドに複数の信頼性向上が行われ、より詳細なエラーメッセージ、より良い例外処理、タイムアウト制御が含まれています。

**関連コミット**: `88da13e`, `a90d080`, `9ef3222`, `9c90675`

**詳細な改善内容については**、[attach コマンドドキュメント](../commands/attach.md)を参照してください。
