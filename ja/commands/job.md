...
`job` コマンドは **非同期コマンドタスクを管理** します：コマンドのライフサイクル確認、結果メタデータの調査、実行中タスクの中断、履歴タスクのクリーンアップを行えます。`watch`、`trace`、`monitor`、`top` などの長時間観測コマンドは通常 job としてバックグラウンドで実行され、`job` はそれらの統一的なコントロール窓口です。

`list`、`status`、`inspect`、`interrupt`、`cleanup` サブコマンドを使えば、target の内部状態に直接触れることなくリソースを回収し、ハングしたタスクを特定し、target / client / ステータスといった複数の軸で履歴を絞り込めます。
